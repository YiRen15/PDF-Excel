...
def parse_single_pdf(pdf_path):
    ...
    try:
        ...
        # 需求三与熔断告警：命中模糊诊断 或 判空，即标记为 ⚠️未分类
        if is_ambiguous_af_afl:
            code_raw = "⚠️未分类"
            code_bracket = "⚠️未分类"
            code_opt = "需要人工复核"
        ...
        # 需求四：第二步——根据诊断判定结果或客观数据，填充负荷与时长数据 (未分类但有客观数据时如实填入，房速严格遵循 >= 30s 规则)
        has_2 = "2" in code_raw or (is_ambiguous_af_afl and ((af_beats > 0) or has_printed_afib_text or has_printed_afib_val))
        has_3 = "3" in code_raw or (is_ambiguous_af_afl and (fl_active or (svt_active and effective_svt_dur_sec >= 30)))
